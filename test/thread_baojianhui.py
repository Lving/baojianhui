...
def __parse(content, num_string):
    # consumer
    tree = HTML(content)
    ...
    if len(detail) == len(key):
        ...
        print(num_string, 'Certificated Number is Not Found')
    else:
        print('Not Matched')
...
def main():
    ...
    # 一个会话给四个线程用？
    # 多个线程在读取验证码的时候是否会出现问题

    for i in range(num):
        t = threading.Thread(target=query, args=(sess, ))
        threads.append(t)
    for i in range(num):
        threads[i].start()
    for i in range(num):
